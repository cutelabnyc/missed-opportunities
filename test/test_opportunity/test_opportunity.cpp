...
void test_silence_op(void)
{
    ...
    uint16_t exp_data[4] = {
        0, 0, 0, 0};
    uint16_t random_reset[4] = {
        0, 0, 0, 0};
    uint16_t density_data[4] = {
        716, 716, 716, 716};

    run_equality_test(&self, (processor_t)OP_process, in_data, out_data, exp_data, random_reset, density_data, msec_register, fixed_msec, 4);
}
...
void test_hysteresis_op(void)
{
    ...
    uint16_t exp_data[8] = {
        0, 0, 0, 0, 0, 0, 1, 1};
    uint16_t random_reset[4] = {0, 0, 0, 0};
    uint16_t density_data[8] = {
        716, 716, 716, 716, 716, 716, 716, 716};

    run_equality_test(&self, (processor_t)OP_process, in_data, out_data, exp_data, random_reset, density_data, msec_register, fixed_msec, 8);
}
...
void test_seed_change(void)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        OP_process(&self, &in_data[i], &exp_data[i], &reset_data[i], &density_data[i], &msec_register[i], fixed_msec);
    }
    for (int i = 0; i < 10; i++)
    {
        OP_process(&self, &in_data[i], &exp_data[i], &reset_data[i], &density_data[i], &msec_register[i], fixed_msec);
    ...
    run_equality_test(&self, (processor_t)OP_process, in_data, out_data, exp_data, reset_data, density_data, msec_register, fixed_msec, 10);
}
...
void test_autopulse(void)
{
    ...
    {
        OP_process(&self, &in_data, &out_data, &reset_data, &density, &autopulse_out, msec);
